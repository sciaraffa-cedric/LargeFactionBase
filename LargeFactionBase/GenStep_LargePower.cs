...
namespace LargeFactionBase
{
    public class GenStep_LargePower : GenStep
    {
        ...
        //private const int MaxDistToExistingNetForTurrets = 4;
        private const int MaxDistToExistingNetForTurrets = 13;

        private const int RoofPadding = 2;

        //private static readonly IntRange MaxDistanceBetweenBatteryAndTransmitter = new IntRange(8, 16);
                    ...
                    if (powerNet == null || !this.HasAnyPowerGenerator(powerNet))
                    {
                        ...
                        if (this.TryFindClosestReachableNet(compPowerBattery.parent.Position, (PowerNet x) => this.HasAnyPowerGenerator(x), map, out powerNet2, out dest))
                        {
                            ...
                            if (this.canSpawnPowerGenerators)
                            {
                                ...
                                float chance = Mathf.InverseLerp((float)GenStep_LargePower.MaxDistanceBetweenBatteryAndTransmitter.min, (float)GenStep_LargePower.MaxDistanceBetweenBatteryAndTransmitter.max, (float)count);
                                Building building;
        ...
        private void SpawnTransmitters(IntVec3 start, IntVec3 end, Map map, Faction faction)
        {
            ...
            if (foundPath)
            {
                map.floodFiller.ReconstructLastFloodFillPath(end, GenStep_LargePower.tmpTransmitterCells);
                this.SpawnTransmitters(GenStep_LargePower.tmpTransmitterCells, map, faction);
            }
        }
        ...
        private bool TrySpawnPowerTransmittingBuildingNear(IntVec3 position, Map map, Faction faction, ThingDef def, out Building newBuilding, Predicate<IntVec3> extraValidator = null)
        {
            ...
            if (RCellFinder.TryFindRandomCellNearWith(position, delegate (IntVec3 x)
            {
                ...
                foreach (IntVec3 current in GenAdj.OccupiedRect(x, Rot4.North, def.size))
                {
        ...
        /*private bool TrySpawnPowerTransmittingBuildingNear(IntVec3 position, Map map, Faction faction, ThingDef def, out Building newBuilding, Predicate<IntVec3> extraValidator = null)
        {
            ...
            if (RCellFinder.TryFindRandomCellNearWith(position, delegate (IntVec3 x)
            {
                ...
                //1.0 CellRect.CellRectIterator iterator = GenAdj.OccupiedRect(x, Rot4.North, def.size).GetIterator();
                CellRect.Enumerator iterator = GenAdj.OccupiedRect(x, Rot4.North, def.size).GetEnumerator();
                //1.0 while (!iterator.Done())
        ...
        /*private bool TrySpawnBatteryNear(IntVec3 position, Map map, Faction faction, out Building newBattery)
        {
            ...
            if (this.spawnRoofOverNewBatteries)
            {
                ...
                {
                    foreach (IntVec3 current in GenAdj.OccupiedRect(x, Rot4.North, ThingDefOf.Battery.size).ExpandedBy(3))
                    {
                        if (current.InBounds(map))
                        {
                            ...
                            for (int i = 0; i < thingList.Count; i++)
                            {
                                ...
                                {
                                    List<PlaceWorker> placeWorkers = thingList[i].def.PlaceWorkers;
                                    Predicate<PlaceWorker> predicate;
                                    if ((predicate = GenStep_LargePower.<> c.<> 9__28_1) == null)
                                    {
                                        predicate = (GenStep_Power.<> c.<> 9__28_1 = new Predicate<PlaceWorker>(GenStep_Power.<> c.<> 9.< TrySpawnBatteryNear > b__28_1));
                                    }
                                    if (placeWorkers.Any(predicate))
                                    {
                                        return false;
                                    }
                                }
